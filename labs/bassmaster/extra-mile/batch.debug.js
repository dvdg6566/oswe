...
internals.process = function (request, requests, resultsData, reply) {
    ...
    Async.series([
        function (callback) {
            ...
            Async.parallel(fnsParallel, callback);// Load modules
...
internals.batch = function (batchRequest, resultsData, pos, parts, callback) {
    ...
    for (var i = 0, il = parts.length; i < il; ++i) {
        ...
        if (parts[i].type === 'ref') {
            ...
            if (ref) {
                var value = null;
                var context = {ref:ref, value:value, parts:parts, i:i};

                try {
                    console.log('Execute: ref.' + parts[i].value + ';');
                    console.log();
                    console.log(JSON.stringify(context));
                    value = eval('ref.' + parts[i].value + ';', context);
                }
                catch (e) {
